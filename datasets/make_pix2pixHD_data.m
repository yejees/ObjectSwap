...
save_dir = sprintf('../imaginaire/dataset/%s_pix2pixHD', object_name);
...
mkdir(sprintf('%s/train/seg_maps',   save_dir));
mkdir(sprintf('%s/val/images',   save_dir));
mkdir(sprintf('%s/val/seg_maps',     save_dir));
mkdir(sprintf('%s/test/images',  save_dir));
mkdir(sprintf('%s/test/seg_maps',    save_dir));
...
for k = 1 : num_data
    ...
    if k <= num_train
        ...
        mkdir(sprintf('%s/train/seg_maps/seq%04d', save_dir, k));
        imwrite(img,    sprintf('%s/train/images/seq%04d/0001.png',   save_dir, k));
        imwrite(img_bg, sprintf('%s/train/seg_maps/seq%04d/0001.png', save_dir, k));
    elseif k <= num_train + num_val
        mkdir(sprintf('%s/val/images/seq%04d',   save_dir, k));
        mkdir(sprintf('%s/val/seg_maps/seq%04d', save_dir, k));
        imwrite(img,    sprintf('%s/val/images/seq%04d/0001.png',   save_dir, k));
        imwrite(img_bg, sprintf('%s/val/seg_maps/seq%04d/0001.png', save_dir, k));
    else
        mkdir(sprintf('%s/test/images/seq%04d',   save_dir, k));
        mkdir(sprintf('%s/test/seg_maps/seq%04d', save_dir, k));
        imwrite(img,    sprintf('%s/test/images/seq%04d/0001.png',   save_dir, k));
        imwrite(img_bg, sprintf('%s/test/seg_maps/seq%04d/0001.png', save_dir, k));
    end
end